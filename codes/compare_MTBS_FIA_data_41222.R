...
## last updated again in May 16 2022
rm(list=ls())
...
trfr_a<-trfr[which(trfr$D1=="ND"|trfr$D1=="NDFD"|
                     trfr$D1=="NDSD"|trfr$D1=="NDTD"),]
trfr_b<-trfr_a[which(trfr_a$T1=="NT"|trfr_a$T1=="NTFT"|
                       trfr_a$T1=="NTST"|trfr_a$T1=="NTTT"),]

trfr_b$D1T1<-interaction(trfr_b$D1,trfr_b$T1)

uvuv<-count(trfr_b,D1T1)


data_dist1<-trfr_b[which(trfr_b$D1T1=="ND.NT"|trfr_b$D1T1=="NDFD.NT"|
                           trfr_b$D1T1=="ND.NTFT"),]

data_dist1$time<-"1"

www11<-count(data_dist1,D1T1)

trees_fia_first<-cbind(data_dist1[,1:147],data_dist1[,294:319])
...
age_tr2<-trees_fia_first1[!is.na(trees_fia_first1$STDAGE.2),]
...
tr$NUNIDD.1<-interaction(tr$NUNID.1,tr$time_i)
tr$NUNIDD.2<-interaction(tr$NUNID.2,tr$time_i)
...
pdam16<-merge(pdam168,pplot21,by="NUNIDS.2")

pdam16_first<-pdam16
write.csv(pdam16_first,"pdam16_first.csv")

rm(data_dist1)
rm(trees_fia_first1)
rm(tr)
data_dist2<-trfr_b[which(trfr_b$D1T1=="ND.NT"|trfr_b$D1T1=="NDSD.NT"|
                           trfr_b$D1T1=="ND.NTST"),]

data_dist2$time<-"2"

www11<-count(data_dist2,D1T1)
trees_fia_second<-cbind(data_dist2[,1:2],data_dist2[,76:220],data_dist2[,294:319])
...
write.csv(pdam16_second,"pdam16_second.csv")
...
www11<-count(data_dist3,D1T1)

trees_fia_third<-cbind(data_dist3[,1:2],data_dist3[,148:293],data_dist3[,294:319])
...
pdam16_third<-pdam16
write.csv(pdam16_third,"pdam16_third.csv")
...
plots_first$time<-"1"
plots_second<-read.csv("pdam16_second.csv")
plots_second$time<-"2"
plots_third<-read.csv("pdam16_third.csv")
plots_third$time<-"3"
...
plots_all$dist_all2<-ifelse(plots_all$dist_all=="0.1.1" & 
                              plots_all$DSTRBCD1.2 %in% fire_code,"0.1.0",
                            ifelse(plots_all$dist_all=="0.1.1" & 
                                     plots_all$DSTRBCD1.2 %in% insdis_code,"0.0.1",
                                   as.character(plots_all$dist_all)))

plots_all$cut1<-ifelse(plots_all$dist_all2=="1.0.0",1,0)
plots_all$fire1<-ifelse(plots_all$dist_all2=="0.1.0",1,0)
