...
# 
# fire<-read.csv("../../data/edited_fire_sev_MTBS.csv") 
# fire2<-fire[c("fire_id","postfirepltcn")]
...
dev.off()
dev.new()
png("mtbs_fig_new31.jpeg", width = 1000, height = 1400)
...
library(gtable)
# legend1 = gtable_filter(ggplot_gtable(ggplot_build(p33 + theme(legend.position="bottom"))), "guide-box")
# # legend1 = gtable_filter(ggplot_gtable(ggplot_build(p13 + theme(legend.position="bottom"))), "guide-box")
# 
# legend2 = gtable_filter(ggplot_gtable(ggplot_build(p36 + theme(legend.position="none"))), "guide-box")
...
p37<-ggplot()+
  ...
  # scale_color_identity(name = " percent of plots",
  #                      breaks = c("black"),
  #                      labels = c("% age zero"),
  #                      guide = "legend")+
  ...
  # theme( axis.title.x.right = element_text(angle = 9,hjust=1))+
...
theme(axis.text.x = element_text(size=14),
  ...
  ggplot()+
    ...
    geom_histogram(data=sdam_consev,aes(diff_bin,fill=age_cat.2),binwidth=1,alpha=0.35)+
  scale_fill_manual(name="Frequency",labels=c("Non-zero","Zero"),
                    values=c("#7570B3","#D95F02"))+
    scale_y_continuous(name = "Frequency of post-disturbance stand age", 
                       sec.axis = sec_axis(~./5, name = "post-disturbance AGB (Mg/ha)" 
                       )) + 
    theme( axis.title.x.right = element_text(angle = 90,vjust=0.5))+

  sdam_consev5<-sdam_consev[which(sdam_consev$age_cat.2=="Zero"),]
...
p38<-ggplot()+
  # geom_bar(stat="identity",fill="gray",alpha=0.5)+
  geom_line(data=hard104,aes(x=diff_bin, y=per.y),linetype="solid",lwd=0.8)+
  # scale_color_identity(name = " percent of plots",
  #                      breaks = c("black"),
  #                      labels = c("% age zero"),
  #                      guide = "legend")+
  ...
  # theme(legend.position="right")+
  geom_histogram(data=sdam_hardsev,aes(diff_bin,fill=age_cat.2),binwidth=1,alpha=0.55)+
  scale_fill_manual(name=NULL,labels=c("plots with non-zero age","plots with zero age"),
                    values=c("gray60","black"))+
  scale_y_continuous(limit=c(0,83),name = "frequency of plots", 
                     sec.axis = sec_axis(~.*1, name = "percent of plots with zero age" 
                     )) + 
  # theme( axis.title.x.right = element_text(angle = 9,hjust=1))+
