...
srpla<-merge(sr,srp,by="NUNID")

srplb<-srpla[which(srpla$DESIGNCD==1),]

srplc<-srplb[which(srplb$CONDID==1),]

sss2<-count(srplc,NUNIDS.x)

srpl<-srplc[which(srplc$PLOT_STATUS_CD==1),]
sss3<-count(srpl,NUNIDS.x)


firstcond<-srpl %>% group_by(NUNID) %>% 
...
sr$cd1<-ifelse(sr$DSTRBCD1==0,"ND",
               ifelse(sr$DSTRBCD1==30|sr$DSTRBCD1==31 | sr$DSTRBCD1==32,"F",
               ifelse(sr$DSTRBCD1<23 & sr$DSTRBCD1>9,"I","O"
              )))



# 
# ggg2<-ggg[which(ggg$cd1=="I"),]
# 
# ggg3<-ggg2[!is.na(ggg2$DSTRBYR1),]
...
sr$tcd1<-ifelse(sr$TRTCD1==0,"NT",
                ifelse(sr$TRTCD1==10,"C","OT"))


dam_all_data<-sr[c("NUNIDS.x","NUNID","MEASYEAR",
                   "cd1","DSTRBYR1","tcd1","TRTYR1",
                   "STDAGE","REMPER","DESIGNCD","KINDCD","ECOSUBCD")]


dam_all_data1<-dam_all_data %>% group_by(NUNID) %>%  slice(1) 


saveRDS(dam_all_data1,"../data/dist_data_all_cond.RDS")

sr$distrt<-interaction(sr$cd1,sr$cd2,sr$cd3,sr$tcd1,sr$tcd2,sr$tcd3)
sr$dis_trt<-as.character(interaction(sr$cd1,sr$tcd1))
...
cn6<-aggregate(dis_trt~NUNID,data=sr,FUN=max)

saveRDS(cn5,"disturbance_plots_all_years.RDS")
