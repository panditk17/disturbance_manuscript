...
mtbs_plots<-read.csv("plots_data_with_fia_mtbs_seedling_f_7_14.csv")
...
# 
#  write.csv(sdam11,"mtbs_plots_FIA1.csv")
# sdam11<-read.csv("mtbs_plots_FIA1.csv")
 sdam11<-mtbs_plots3

sdam11$diff_yr<-sdam11$MEASYEAR.2-as.numeric(sdam11$fire_yr)
sdam11$diff_mn<-sdam11$MEASMON.2-as.numeric(sdam11$fire_mn)
...
sdam12$AGB_CHP<-sdam12$AGB.22-sdam12$AGB.11
...
library(ggplot2)
ggplot(data=sdam12) +
  geom_jitter(aes(x=diff_years, y=STDAGE.2,color=as.factor(mtbs_sev))) +
...
ggplot(data=sdam12) +
  geom_jitter(aes(x=diff_years, y=AGB.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
...
ggplot(data=sdam12) +
  geom_jitter(aes(x=diff_years, y=seed_count.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
    # scale_y_continuous(limits=c(0,500))+
  xlab("time since fire (days)")+
...
ggplot(data=sdam12) +
  geom_jitter(aes(x=diff_months, y=STDAGE.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
...
ggplot(data=dam_westsev) +
  geom_jitter(aes(x=diff_months, y=STDAGE.2,color=as.factor(Spl_1))) +
...
ggplot(data=sdam_con) +
  geom_point(aes(x=diff_months, y=AGB.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
  # scale_y_continuous(limits=c(0,500))+
  xlab("time since fire (days)")+
...
ggplot(data=sdam_consev) +
  geom_point(aes(x=diff_years, y=STDAGE.2,color=as.factor(mtbs_sev))) +
...
p31<-ggplot(data=sdam_consev) +
  ...
  theme(axis.text.x = element_text(size=14),
        ...
        legend.title=element_text(size=14),
        legend.text = element_text(size=13)) +
  geom_text(x=3, y=10, label="Stand age increasing: 27 % of plots",size=4.7)+
  
  geom_text(x=3, y=9, label="Stand age 0 persistent: 73 % of plots",size=4.7)
...
p32<-ggplot(data=sdam_hardsev) +
  ...
  theme(axis.text.x = element_text(size=14),
        ...
        legend.title=element_text(size=14),
        legend.text = element_text(size=13))+
  geom_text(x=3, y=10, label="Stand age increasing: 81 % of plots",size=4.7)+
  
  geom_text(x=3, y=9, label="Stand age 0 persistent: 19 % of plots",size=4.7)
# sdam_consev$age<-ifelse(sdam_consev$STDAGE.2>0,1,0)
# sdam_hardsev$age<-ifelse(sdam_hardsev$STDAGE.2>0,1,0)
...
table(sdam_consevage0$FLDSZCD.2,sdam_consevage0$STDAGE.2)


sdam_hardsevage0<-sdam_hardsev[which(sdam_hardsev$STDAGE.2==0),]
table(sdam_hardsevage0$FLDSZCD.2,sdam_hardsevage0$STDAGE.2)
