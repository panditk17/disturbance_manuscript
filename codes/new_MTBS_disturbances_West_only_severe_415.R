...
sdam12$AGB.1<-(sdam12$AGB.1*453.6*2.471)/1000000
sdam12$AGB.2<-(sdam12$AGB.2*453.6*2.471)/1000000

sdam12$AGB_CHP<-sdam12$AGB.2-sdam12$AGB.1
sdam12<-sdam12[which(sdam12$STDAGE.1>=20),]
sdam12<-sdam12[which(sdam12$STDSZCD.1<5),]
...
ggplot(data=mtbs_seed) +
  geom_jitter(aes(x=diff_days, y=STDAGE.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
  scale_y_continuous(limits=c(0,15))+
  xlab("time since fire (years)")+
...
ggplot(data=sdam_con) +
  geom_point(aes(x=diff_days, y=AGB.2,color=as.factor(mtbs_sev))) +
  scale_color_manual(name="fire severity",labels=c("mild","severe"), 
                     values=c("0"="gold1","1"="red")) + 
  # scale_y_continuous(limits=c(0,500))+
  xlab("time since fire (days)")+
...
ggplot(data=sdam_hard) +
  geom_point(aes(x=diff_days, y=AGB_CHP,color=as.factor(mtbs_sev))) +
  geom_smooth(aes(x=diff_days, y=AGB_CHP,color=as.factor(mtbs_sev)),methods=loess) +
...
scale_y_continuous(limit=c(0,47),breaks=c(seq(0,60,10)),name = "number of plots") +
theme_bw()+
...
p38<-ggplot(sdam_hardsev,aes(x=diff_bin,group=age_cat.2,fill=age_cat.2))+
  ...
  scale_fill_manual(labels=c(" plots with non-zero age"," plots with zero age"),
                    values=c("gray60","black"))+
  scale_y_continuous(limit=c(0,47),breaks=c(seq(0,60,10)),name = "") +
  theme_bw()+
  theme(legend.position = c(0.7,0.85))+
  xlab("")+
  ylab("% of plots")+
