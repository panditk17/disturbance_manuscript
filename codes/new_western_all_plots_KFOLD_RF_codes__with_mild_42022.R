...
# 
# write.csv(plots_all2,"plots_seedling_disturbance_FIA_new_with_cond_plot.csv")
...
sdam42$DISTURB<-as.factor(sdam42$distrt_his)
...
model_fire_agb_kf<-train(y = learn_fire_agb$AGB.2,
                        ...
                        method="rf",
                        trControl=fitcontrol1,tuneGrid=tunegrid1,ntree=1000)
...
sdam8<-sdam5b[which(sdam5b$dist_type=="insdis"),]


data_insdis_agb1<-sdam8[c("AGB.2","AGB.1","STDORG","FORGRP","STDAGE.1","STDAGE.2",
                       "ECOREG","ELEV","aspect_trans","SLOPE","phy_fac")]


colnames(data_insdis_agb1)<-c("AGB.2","Pre_dist_AGB","Stand_origin","Forest_Group",
                           "Pre_dist_stand_age","Post_dist_stand_age",
                           "Ecoregion","elevation","aspect","slope",
                           "physiography")


data_insdis_agb<-na.omit(data_insdis_agb1)

set.seed(51)

train_insdis_agb<-sample(1:nrow(data_insdis_agb),0.80*(nrow(data_insdis_agb)))
valid_insdis_agb<-data_insdis_agb[-train_insdis_agb,]
learn_insdis_agb<-data_insdis_agb[train_insdis_agb,]


tunegrid1 <- expand.grid(.mtry = (1:10)) 
model_insdis_agb_kf<-train(y = learn_insdis_agb$AGB.2,
                        x = learn_insdis_agb[,colnames(learn_insdis_agb)!= "AGB.2"],
                        method="rf",
                        trControl=fitcontrol1,tuneGrid=tunegrid1,ntree=1000)

print(model_insdis_agb_kf)
varImp(model_insdis_agb_kf)
model_insdis_agb_kf$finalModel



curr_date<-Sys.Date()
saveRDS(model_insdis_agb_kf, file=paste0("model_west_insdis_agb_kf_",curr_date,".RDS"))
saveRDS(model_cut_agb_kf, file=paste0("model_west_cut_agb_kf_",curr_date,".RDS"))
saveRDS(model_fire_agb_kf, file=paste0("model_west_fire_agb_kf_",curr_date,".RDS"))
saveRDS(model_all_agb_kf, file=paste0("model_west_all_agb_kf_",curr_date,".RDS"))
# 
# curr_date<-"2022-04-20"
# model_west_insdis_agb_kf<-readRDS(file=paste0("model_west_insdis_agb_kf_",curr_date,".RDS"))
# saveRDS(model_cut_agb_kf, file=paste0("model_west_cut_agb_kf_",curr_date,".RDS"))
# saveRDS(model_fire_agb_kf, file=paste0("model_west_fire_agb_kf_",curr_date,".RDS"))
# saveRDS(model_all_agb_kf, file=paste0("model_west_all_agb_kf_",curr_date,".RDS"))
...
data_insdis_seed1<-sdam8[c("seed_count.2","AGB.1","STDORG","FORGRP","STDAGE.1","STDAGE.2",
                        "ECOREG","ELEV","aspect_trans","SLOPE","phy_fac")]


colnames(data_insdis_seed1)<-c("seed_count.2","Pre_dist_seedling_density","Stand_origin","Forest_Group",
                            "Pre_dist_stand_age","Post_dist_stand_age",
                            "Ecoregion","elevation","aspect","slope",
                            "physiography")


data_insdis_seed<-na.omit(data_insdis_seed1)

set.seed(51)

train_insdis_seed<-sample(1:nrow(data_insdis_seed),0.80*(nrow(data_insdis_seed)))
valid_insdis_seed<-data_insdis_seed[-train_insdis_seed,]
learn_insdis_seed<-data_insdis_seed[train_insdis_seed,]


tunegrid1 <- expand.grid(.mtry = (1:10)) 
model_insdis_seed_kf<-train(y = learn_insdis_seed$seed_count.2,
                         x = learn_insdis_seed[,colnames(learn_insdis_seed)!= "seed_count.2"],
                         method="rf",
                         trControl=fitcontrol1,tuneGrid=tunegrid1,ntree=1000)

print(model_insdis_seed_kf)
varImp(model_insdis_seed_kf)
model_insdis_seed_kf$finalModel




curr_date<-Sys.Date()
saveRDS(model_insdis_seed_kf, file=paste0("model_west_insdis_seed_kf_",curr_date,".RDS"))
saveRDS(model_cut_seed_kf, file=paste0("model_west_cut_seed_kf_",curr_date,".RDS"))
saveRDS(model_fire_seed_kf, file=paste0("model_west_fire_seed_kf_",curr_date,".RDS"))
saveRDS(model_all_seed_kf, file=paste0("model_west_all_seed_kf_",curr_date,".RDS"))
