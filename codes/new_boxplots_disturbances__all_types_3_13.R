...
# sdam1<-sdam1[which(sdam1$STDAGE_2<2000),]
...
write.csv(all_sev_dist,"all_sev_dist.csv")
write.csv(all_mild_dist,"all_mild_dist.csv")
...
ggplot(aes(x=as.factor(type), y=mean, fill=time),stat="identity") +
  ...
  geom_text(x=0.5, y=135, label="(a)",size=6.5)+
...
p2<-sev_agb1 %>%
  ...
  geom_text(x=0.5, y=175, label="(a)",size=6.5)+
...
p3<-mild_seed1 %>%
  ...
  geom_text(x=0.5, y=3200, label="(c)",size=6.5)+
...
p4<-sev_seed1 %>%
  ...
  geom_text(x=0.5, y=6730, label="(c)",size=6.5)+
...
rm(list = setdiff(ls(), c('p1','p2','p3','p4','table_west','table_west_m')))
...
freq_sev_eastern<-count(all_sev_dist,type,time)
freq_mild_eastern<-count(all_mild_dist,type,time)
...
colnames(sevstat6)<-c("class","n_east","AGB_med_east","AGB_mean_east",
...
p15<-mild_agb1 %>%
  ...
  geom_text(x=0.5, y=136, label="(b)",size=6.5)+
  theme_bw()+
  theme(legend.position="none")+
  theme(axis.text.x = element_text(size=17),
        axis.text.y = element_text(size=17),
...
p16<-sev_agb1 %>%
  ...
  geom_text(x=0.5, y=106, label="(b)",size=6.5)+
...
p17<- mild_seed1 %>%
  ...
  geom_text(x=0.5, y=5900, label="(d)",size=6.5)+
...
p18<-sev_seed1 %>%
  ...
  geom_text(x=0.5, y=11900, label="(d)",size=6.5)+
